...
<main>
	<section class="hero" aria-labelledby="hero">
        ...
        <a href="#presentation"
           class="hero__link btn"><?= __('Décourvir Antilope', 'ant') ?></a>
        <div class="scroll-down">
            <p><strong>Faites défiler la page</strong> pour en savoir plus</p>
    ...
    <section class="modules" aria-labelledby="modules">
	        ...
	        if(($modules = ant_get_modules(3))->have_posts()): while($modules->have_posts()): $modules->the_post()?>
                <article class="module" aria-labelledby="<?= get_post_field('post_name'); ?>">
                    <div class="module__card">
                        ...
                        <a href="<?= get_the_permalink(); ?>" class="module__link full__link"><?= str_replace(':title', get_the_title(), __('Voir le module', 'ant')); ?></a>
                    </div>
                </article>
	        <?php endwhile; else: ?>
        ...
        <a href="<?= get_post_type_archive_link('module'); ?>"
           class="modules__link"> <?= __('Voir tous les modules', 'ant') ?></a>
    </section>
